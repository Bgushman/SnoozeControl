...
export default function Calibration() {
  ...
  const liveEarRef = useRef(0.28); // so sampler closure always reads latest
  ...
  // -----------------------------
  // EAR ticker — real endpoint with mockEar() fallback
  // -----------------------------
  useEffect(() => {
  ...
  // Use liveEarRef so the interval closure always gets the latest EAR
  ...
  // Send thresholds to model endpoint
  try {
    await fetch("http://172.31.181.137:8080/calibration", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ earOpen, earClosed, earThreshold }),
    });
  } catch {
    // Endpoint unreachable — thresholds still saved locally, no crash
  }
  ...
  return (
    <View style={styles.screen}>
      {/* pointerEvents="none" prevents CameraView blocking panel touches */}
      <CameraView style={styles.camera} facing="front" pointerEvents="none" />
      ...
      >
        ...
        <View style={styles.hudRow}>
          <Text style={styles.hudText}>
            Live EAR {liveEar.toFixed(3)}
            {"  "}
            <Text style={{ color: connected ? "#2bb24c" : "#888", fontSize: 13 }}>
              {connected ? "• Connected" : "• Mock"}
            </Text>
          </Text>
          {isRecording && <ActivityIndicator color="#2f80ff" style={{ marginTop: 4 }} />}
        </View>

        <Pressable
          style={[styles.bigBtn, styles.blueBtn, isRecording && styles.btnDisabled]}
          onPress={() =>
        ...
        <View style={styles.card}>
          <Row label="EAR Open" value={openAvg?.toFixed(3) ?? "--"} />
          <Row label="EAR Closed" value={closedAvg?.toFixed(3) ?? "--"} />
          <Row label="Threshold" value={threshold?.toFixed(3) ?? "--"} last />
        </View>

        {step === 1 ? (
          <Pressable
            style={[styles.bigBtn, styles.greenBtn, !canNext && styles.btnDisabled]}
            onPress={() => setStep(2)}
            disabled={!canNext}
          >
          ...
          <Pressable
            style={[styles.bigBtn, styles.greenBtn, !canSave && styles.btnDisabled]}
